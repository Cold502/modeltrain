...
export async function createSSEStream(url, requestData, onChunk, onComplete, onError) {
  try {
    logSafe('🌐 准备发送SSE请求到:', { url })
    logSafe('📄 请求体:', requestData)
    ...
    logSafe('📡 收到响应，状态码:', { status: response.status })
...
async function processSSEResponse(response, onChunk, onComplete, onError) {
  ...
  try {
    while (true) {
      ...
      // 只在调试模式下输出详细信息
      if (chunkCount % 10 === 0) { // 每10个块输出一次
        log(`📦 已读取${chunkCount}个数据块，缓冲区长度: ${buffer.length}`)
      }
...
export function debugRefreshToken() {
  log('🔍 检查Refresh Token状态:')
  
  // 检查localStorage中的token
  const accessToken = localStorage.getItem('token')
  logSafe('📱 localStorage中的access_token:', { exists: !!accessToken })
  ...
  logSafe('🍪 Cookie中的refresh_token:', { exists: !!refreshTokenCookie })
